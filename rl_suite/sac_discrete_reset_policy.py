...
from rl_suite.algo.replay_buffer import SACReplayBuffer, ResetSACBuffer
from rl_suite.experiment import Experiment
from rl_suite.running_stats import RunningStats


class InvokeResetSACDiscrete(Experiment):
    def __init__(self):
        super(InvokeResetSACDiscrete, self).__init__(self.parse_args())
    ...
    def run(self):
        ...
        rets = []
        ep_lens = []
        reset_tracker = np.zeros((2, self.args.N))        
        i_episode = 0
        t = 0
        while t < self.args.N:
            done = False
            ...
            while not done:
                if self.args.algo == "sac_rad":
                    ...
                    reset_obs = self.add_time_to_obs(obs, reset_step)
                ...
                if self.args.algo == "sac":
                    ...
                    reset_learner.push_and_update(reset_obs, reset_action, r, done)
                else:
                ...
                if (t+1) % self.args.checkpoint == 0:
                    if rets:
                        ...
                        np.savetxt(self.base_fname + "num_resets.txt", np.array(reset_tracker))
